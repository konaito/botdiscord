...
async def userinfo(interaction: discord.Interaction, user: discord.Member = None):
    ...
    embed.add_field(name="アカウント作成日", value=user.created_at.strftime("%Y年%m月%d日"), inline=True)
    embed.add_field(name="サーバー参加日", value=user.joined_at.strftime("%Y年%m月%d日"), inline=True)
...
@app.get("/")
async def root():
    return {"message": "Discord Bot API Server", "status": "running"}
...
async def handle_discord_interaction(interaction: DiscordInteraction):
    ...
    try:
        if interaction.type == 1:  # PING
            ...
            return {"type": 4, "data": {"content": "不明なインタラクションタイプです。"}}
